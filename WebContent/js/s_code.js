...
/*OPTIMICS LOG*/
/*
 *13/4/11 Conversion Vars, Traffic Vars, Std Vars
 *18/4/11 s_account pre-setup detection, page_name
 *26/4/11 s_account new production RSID avast-web replaces av-web 
 *26/4/11 s.prop19 moved to HTML
 *26/4/11 trackingServer(Secure) set to FPC
 *02/5/11 s.eVar5,s.prop5 moved from HTML to scode
 *03/5/11 BUGFIX#1 s.hier1 gn->pageName
 *09/5/11 BUGFIX#2 eVar12,13,14,15,27;getValOnce->cookie
 *11/5/11 BUGFIX#2 eVar12,13,14,15,27;getValOnce->cookie (A)
 *11/5/11 s_account production subdomains logic updated (D)
 *11/5/11 s.linkInternalFilters digitalriver.com added (C)
 *11/5/11 FPC avast.co.jp (E)
 *11/5/11 s.eVar11,s.prop11 added (B)
 *12/5/11 added prefix osc_ to cookie names
 *16/5/11 BUGFIX#3 eVar14 (line 176)
 *16/5/11 Cookies have path parameter '/' added
 *18/5/11 Value of eVar15 unescaped (lines 185,187)
 *19/5/11 Interstitials tracking added: s.events+s.eVar23
 *28/6/11 TT to SC integration (trackTNT plugin)
 **/
 
 /*AVAST LOG*/
/*
 *24/5/11 s_account production subdomains logic updated (Curtis)
 *24/5/11 BUGFIX#4 prop21,22 (lines 318,320) (Curtis)
 *30/6/11 Added ': ' prefix to eVar31,35,36 to allow SAINT classification (Curtis)
 *16/9/11 Revised Source and LOB parameter logic for case when cookies defined
 *19/9/11 BUGFIX#5 eVar27 corrected for email allocation
 *24/9/11 Added LOB case for Free | Not Registered
 *20/2/12 Add order tracking cookie code (Curtis)
 *13/4/12 Add eVar42 with p_pro and p_lqe (Milan)
...
if(typeof s_account == "undefined") {
    var s_account
    if (window.location.hostname == "www.avast.com" ||
	    window.location.hostname == "store.avast.ru" ||
        window.location.hostname == "blog.avast.com" ||
        window.location.hostname == "forum.avast.com" ||
        window.location.hostname == "support.avast.com" ||
        window.location.hostname == "onlinescan.avast.com" ||
        window.location.hostname == "www.avast.co.jp" ||
        window.location.hostname == "dashboard.ca.avast.com" ||
        window.location.hostname == "activation.ca.avast.com"
		) {
        //production suite
        s_account="avast-web";
    } else {
...
if (window.location.hostname == "www.avast.co.jp" ) {
    ...
    s.trackingServer="an.avast.com";
    s.trackingServerSecure="ans.avast.com";
}
...
function s_doPlugins(s) {
    ...
    if(typeof s.pageName == "undefined" && page_name) {
        s.pageName=page_name;
    }

    /* if campaign doesn't have a value, get omcid from the query string */
    ...
    /*********************** CONVERSION VAR SECTION ************************/
    
    /*s.eVar1 - Server*/
    s.eVar1="D=server";
    /*s.eVar2 - Site Locale (HTML)*/
    /*s.eVar3 - Site Section - if channel set, copy to eVar3 */
    if(s.channel)
        s.eVar3="D=ch";
    /*s.eVar4 - Pagename - if pagename set, copy to eVar4 */
    if(s.pageName)
        s.eVar4="D=pageName";
    /*s.eVar5 - Page Variation*/
    if(typeof page_version != "undefined"){
        s.eVar5=window.location.host + window.location.pathname + " " + page_version
    }
    else {
    ...
    if(s.getQueryParam('p_tid')){
	s.eVar6 = s.getQueryParam('p_tid');
    }else if(typeof s_tev == "undefined") {
    	if (rc('osc_v6')){
    ...
    if (typeof s.eVar6 != "undefined"){
        ...
        } else if (window.location.hostname.search("store.avast.ru") != -1){
    ...
    if(s.getQueryParam('tid'))
        s.eVar8=s.getQueryParam('tid');
    /*s.eVar9 - IP Country (VISTA)*/
    /*s.eVar10 - New vs Repeat Visitor - return getNewRepeat plug-in to eVar10*/
    s.eVar10=s.getNewRepeat(365,'s_nr2');
    /*s.eVar11 Language*/
    if(typeof(locale) != "undefined"){
        s.eVar11=locale.substr(0,2);
    }
    /*s.eVar12 - Source Group */
    if(rc("osc_v12"))
		s.eVar12=rc("osc_v12");
	else {
		if(parseInt(s.getQueryParam('p_pro')) === 0)
            s.eVar12="Free Program";
        ...
        if(window.location.hostname.search("www.avast.co.jp") != -1)
            ...
            document.cookie="osc_v12="+s.eVar12+";domain=.avast.com;path=/;";
	}
    /*s.eVar13	Source Channel*/
    if(rc("osc_v13"))
		s.eVar13=rc("osc_v13");
	else {
		if(parseInt(s.getQueryParam('p_pro')) === 0) {
            if(parseInt(s.getQueryParam('p_lst')) === 0 || parseInt(s.getQueryParam('p_lst')) === 1 || parseInt(s.getQueryParam('p_lst')) === 2)
                s.eVar13="Free Program | Registered";
            else if(parseInt(s.getQueryParam('p_lst')) === 3 || parseInt(s.getQueryParam('p_lst')) === 4 || parseInt(s.getQueryParam('p_lst')) === 5)
                s.eVar13="Free Program | Not Registered";
        }
        else if(parseInt(s.getQueryParam('p_pro')) > 0)    {
            if(parseInt(s.getQueryParam('p_lst')) === 0 || parseInt(s.getQueryParam('p_lst')) === 1 || parseInt(s.getQueryParam('p_lst')) === 2)
                s.eVar13="Paid Program | Licensed";
            else if(parseInt(s.getQueryParam('p_lst')) === 3 || parseInt(s.getQueryParam('p_lst')) === 4 || parseInt(s.getQueryParam('p_lst')) === 5)
                s.eVar13="Paid Program | Trial";
        }
        else if(s.getQueryParam('cha') == "ppc")
            s.eVar13="Website | Paid Search";
        else if(s.getQueryParam('cha') == "aff")
            s.eVar13="Website | Affiliate";
        else if(s.getQueryParam('cha') == "ema")
            s.eVar13="Website | Email";
        ...
        if(window.location.hostname.search("www.avast.co.jp") != -1)
            ...
            document.cookie="osc_v13="+s.eVar13+";domain=.avast.com;path=/;";
	}
    /*s.eVar14 - Source Element*/
	if(rc("osc_v14"))
		s.eVar14=rc("osc_v14");
	else if(s.eVar13) {
		if(s.getQueryParam('p_elm'))
            s.eVar14=s.eVar13.concat(" | ", s.getQueryParam('p_elm'));
        ...
        if(window.location.hostname.search("www.avast.co.jp") != -1)
            ...
            document.cookie="osc_v14="+s.eVar14+";domain=.avast.com;path=/;";
	}
    /*s.eVar15 - Source Variation*/
    if(rc("osc_v15"))
		s.eVar15=rc("osc_v15");
	else if(s.eVar13) {
		if(s.getQueryParam('p_var'))
            s.eVar15=s.eVar13.concat(" | ", unescape(s.getQueryParam('p_var')));
        ...
        if(window.location.hostname.search("www.avast.co.jp") != -1)
            ...
            document.cookie="osc_v15="+s.eVar15+";domain=.avast.com;path=/;";
	}
    /* s.eVar16 - Search Engine - copy URL param 'sen' to eVar16 */
    if(s.getQueryParam('sen'))
        s.eVar16=s.getQueryParam('sen');
    /*s.eVar17 - Search Term - copy URL param 'ste' to eVar17 */
    if(s.getQueryParam('ste'))
        s.eVar17=s.getQueryParam('ste');
    /*s.eVar18 - Internal Search Term - copy URL param 'query' to eVar18 */
    if(s.getQueryParam('query'))
        s.eVar18=s.getQueryParam('query');
    /*s.eVar19 - Null Search Term (HTML)*/
    /*s.eVar20 - Affiliate ID - copy URL param 'aid' to eVar20 */
    if(s.getQueryParam('aid')) {
        s.eVar20=s.getQueryParam('aid');
    }
    /*s.eVar21 - Display Currency(HTML)*/
    /*s.eVar22 - Payment Type(HTML)*/
    /*s.eVar23 - Offer Item*/
    if(typeof s_eVar23 != "undefined") {
        s.eVar23=s_eVar23;
    }
    /*s.eVar24 - Download Server(HTML)*/
    /*s.eVar25 - Download Name(HTML)*/
    /*s.eVar26 - Download URL(HTML)*/
    /*s.eVar27 - Line of Business*/
    if(rc("osc_v27"))
		s.eVar27=rc("osc_v27");
	else {
	  if(s.getQueryParam('p_bld') == "FreeInBusiness")
        ...
        if(window.location.hostname.search("www.avast.co.jp") != -1)
            ...
            document.cookie="osc_v27="+s.eVar27+";domain=.avast.com;path=/;";
	}
    /*s.eVar28, s.eVar29 NOT USED*/
    /*s.eVar30*/
    if(s.getQueryParam('p_cid')){
	s.eVar30 = s.getQueryParam('p_cid');
    }else {
    	if (rc('osc_v30')){
        	// 2. !s_tev && rc(osc_v6) =>  s.eVar6 = rc(osc_v6)
    		s.eVar30 = rc('osc_v30');
    ...
    if (typeof s.eVar30 != "undefined"){
        if(window.location.hostname.search("www.avast.co.jp") != -1) {
            ...
            document.cookie="osc_v30="+s.eVar30+";domain=.avast.com;path=/;";
        }
    }
    
    /*s.eVar31 - Product Type - copy URL param 'p_pro' to eVar31 */
    if(s.getQueryParam('p_pro'))
        s.eVar31=": " + s.getQueryParam('p_pro');
    /*s.eVar32 - Product Version - copy concat URL params 'p_vep' and 'p_ves' to eVar32 */
    if(s.getQueryParam('p_vep') && s.getQueryParam('p_ves'))
        s.eVar32=s.getQueryParam('p_vep').concat("_",s.getQueryParam('p_ves'));
    /*s.eVar33 - License Count - copy URL param 'p_lqa' to eVar33 */
    if(s.getQueryParam('p_lqa'))
        s.eVar33=s.getQueryParam('p_lqa');
    /*s.eVar34 - Subscription Length - copy URL param 'p_lsu' to eVar34 */
    if(s.getQueryParam('p_lsu'))
        s.eVar34=s.getQueryParam('p_lsu');
    /*s.eVar35 - License Stage - copy URL param 'p_lst' to eVar35 */
    if(s.getQueryParam('p_lst'))
        s.eVar35=": " + s.getQueryParam('p_lst');
    /*s.eVar36 - Days to Expiration - copy URL param 'p_lex' to eVar36 */
    if(s.getQueryParam('p_lex'))
        s.eVar36=": " + s.getQueryParam('p_lex');
    /*s.eVar37 - Product Language - copy URL param 'p_lng' to eVar37 */
    if(s.getQueryParam('p_lng'))
        s.eVar37=s.getQueryParam('p_lng');
    /*s.eVar38	OS Locale  - copy URL param 'p_lid' to eVar38 */
    if(s.getQueryParam('p_lid'))
        s.eVar38=s.getQueryParam('p_lid');
	...
	if(s.getQueryParam('p_pro')){
    	var ppro = s.getQueryParam('p_pro');
    	var ppros ={
    			"0":"fa",
    			"1":"pa",
		    	"2":"is",
    	...
    	var plqe = s.getQueryParam('p_lqe');
    	var plqeT = "";
    	if(plqe){
    		plqeT = plqe;
    	}
    	s.eVar42=pproT+"-"+plqeT;
    }else{
    	s.eVar42="web";
    }
    ...
    if(s.eVar1)
        s.prop1="D=v1";
    /*s.prop2 - Site Locale (HTML)*/
    /*s.prop5 - Page Variation*/
    if(s.eVar5)
        s.prop5="D=v5";
    /*s.prop3, s.prop4, s.prop6 NOT USED*/
    if(s.eVar7)
        s.prop7="D=v7";
    if(s.eVar8)
        s.prop8="D=v8";
    /*s.prop9 - IP Country (VISTA)*/
    if(s.eVar10)
        s.prop10="D=v10";
    if(s.eVar11)
        s.prop11="D=v11";
    ...
    if(s.eVar18)
        s.prop18="D=v18";
    /*s.prop19 - Null Search Term (HTML)*/
    /*s.prop20 NOT USED*/
    if(s.eVar2 && s.pageName)
        s.prop21='D=v2+": "+pageName';
    if(rc("osc_v12"))
        s.prop22=rc("osc_v12")+": "+s.pageName;

    /*********************** HIER VAR SECTION ************************/

    s.hier1='D=ch+">"+pageName+">"+v5';
    if(typeof s.eVar12 != "undefined" &&
        typeof s.eVar13 != "undefined" &&
        typeof s.eVar14 != "undefined" &&
        typeof s.eVar15 != "undefined")
        s.hier2='D=v12+">"+v13+">"+v14+">"+v15';
	
	s.tnt=s.trackTNT();
	
    /*************** ORDER TRACKING PARAMETER SECTION ****************/
	
	/* Test if cookie is already present */
	if(!rc("osc_ot")) {
    ...
    if(typeof s.eVar6 != "undefined")
      ...
      tid = "un";
		...
		if(window.location.hostname.search("www.avast.co.jp") != -1) {
			...
			document.cookie="osc_ot="+c_ot+";domain=.avast.ru;path=/;";
			document.cookie="x-otid="+c_ot+";domain=.avast.ru;path=/;";
		} else {
			document.cookie="osc_ot="+c_ot+";domain=.avast.com;path=/;";
